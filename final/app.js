...
function setup() {
  ...
  for (let z = 0; z < 6; z++) {
    ...
    // console.log('image a', cardObj.imageA)
    // console.log('image b', cardObj.imageB)
    selectedFaces.push({ image: cardObj.imageA, name: cardObj.name }); // making this into an object and passing name
    selectedFaces.push({ image: cardObj.imageB, name: cardObj.name }); // ditto, so we can match on this
  ...
  for (let j = 0; j < 2; j++) {
    for (let i = 0; i < 6; i++) { //# of cards
      ...
      cards.push(new Card(startingX, startingY, faceImage.image, faceImage.name));
...
function mousePressed() {
  ...
  if (gameState.flippedCards.length === 2) {
    gameState.attepts++;
    if (gameState.flippedCards[0].name === gameState.flippedCards[1].name) { // changing this b/c images won't match but now name will
...
class Card {
  constructor(x, y, cardFaceImg, name) {
    this.x = x;
    this.y = y;
    ...
    this.name = name;
    this.face = DOWN;
    console.log('face image', cardFaceImg);
